...
def generar_reporte(df, counts, freq, desviacion, calor):
    ...
    with open(REPORT_FILE, "w", encoding="utf-8") as f:
        ...
        f.write(f"_Actualizado: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n")
        f.write("---\n\n")
        f.write("**¿Qué es Tris?**\n\n")
        ...
        if len(df) > 0:
            ...
            calientes = sum(1 for num in ultimo if calor.get(num) == "🌡️ Caliente")
            normales = sum(1 for num in ultimo if calor.get(num) == "➡️ Normal")
            frios = sum(1 for num in ultimo if calor.get(num) == "❄️ Frío")
        ...
        # Ejemplo: Selección balanceada
        calientes = [num for num in range(10) if calor.get(num) == "🔥 Muy caliente" or calor.get(num) == "🌡️ Caliente"]
        frios = [num for num in range(10) if calor.get(num) == "🧊 Muy frío" or calor.get(num) == "❄️ Frío"]
        normales = [num for num in range(10) if calor.get(num) == "➡️ Normal"]
        f.write(f"- Estrategia balanceada: {calientes[:2] + normales[:1] + frios[:2]}\n")
        f.write(f"- Estrategia calientes: {calientes[:5]}\n")
        f.write(f"- Estrategia fríos: {frios[:5]}\n")
        f.write(f"- Estrategia aleatoria: {np.random.choice(range(10), 5, replace=False).tolist()}\n\n")
